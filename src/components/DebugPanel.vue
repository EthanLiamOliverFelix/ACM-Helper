...
.debug-panel { flex: 1; min-height: 0; display: flex; flex-direction: column; background: var(--color-bg-app); color: var(--color-tone-ccc); }
.debug-panel__header { display: flex; align-items: center; justify-content: space-between; padding: 11px 13px; border-bottom: 1px solid var(--color-border-soft); div { display: flex; flex-direction: column; gap: 2px; } strong { color: var(--color-tone-eee); font-size: 14px; } span { color: var(--color-text-faint); font-size: 9px; } button { padding: 5px 9px; border: 1px solid var(--color-tone-814242); border-radius: 4px; background: var(--color-tone-3b2525); color: var(--color-tone-f1a3a3); cursor: pointer; } }
.debug-panel__body { flex: 1; min-height: 0; overflow-y: auto; padding: 12px; }
.debug-panel__label { display: block; margin-bottom: 5px; color: var(--color-text-soft); font-size: 11px; font-weight: 600; }
textarea { box-sizing: border-box; width: 100%; min-height: 92px; max-height: 210px; resize: vertical; padding: 8px; border: 1px solid var(--color-border-control); border-radius: 5px; outline: none; background: var(--color-bg-deep); color: var(--color-text-strong); font: 11px/1.45 Consolas, monospace; &:focus { border-color: var(--color-tone-6d4d91); } &:disabled { color: var(--color-tone-888); } }
.debug-panel__start { width: 100%; margin-top: 8px; padding: 9px; border: 0; border-radius: 5px; background: var(--color-accent-strong); color: var(--color-text-on-accent); font-weight: 600; cursor: pointer; &:disabled { opacity: .45; cursor: not-allowed; } }
.debug-panel__status { display: flex; justify-content: space-between; gap: 6px; margin-top: 8px; padding: 7px 8px; border-left: 3px solid var(--color-purple); background: var(--color-tone-2b2234); color: var(--color-tone-dcc9ef); font-size: 10px; &.ended { border-color: var(--color-text-faint); background: var(--color-bg-control-alt); color: var(--color-text-soft); } }
.debug-panel__error { margin-top: 8px; padding: 8px; border: 1px solid var(--color-tone-713838); border-radius: 4px; background: var(--color-tone-351d1d); color: var(--color-danger); font-size: 10px; line-height: 1.45; word-break: break-word; }
.debug-panel__controls { display: grid; grid-template-columns: repeat(4, 1fr); gap: 5px; margin: 10px 0 14px; button { min-width: 0; padding: 7px 2px; border: 1px solid var(--color-tone-4c3b5c); border-radius: 4px; background: var(--color-tone-30243b); color: var(--color-purple); font-size: 15px; cursor: pointer; span { display: block; margin-top: 2px; font-size: 9px; } &:disabled { opacity: .35; cursor: not-allowed; } } }
.watch-section, .debug-output { margin-top: 12px; border-top: 1px solid var(--color-tone-353535); padding-top: 10px; h3 { margin: 0 0 7px; color: var(--color-text-secondary); font-size: 11px; } }
.watch-section__add { display: flex; gap: 5px; margin-bottom: 6px; input { min-width: 0; flex: 1; padding: 6px 7px; border: 1px solid var(--color-border-control); border-radius: 4px; outline: none; background: var(--color-bg-deep); color: var(--color-text-strong); font: 10px Consolas, monospace; &:focus { border-color: var(--color-tone-6d4d91); } } button { width: 28px; border: 1px solid var(--color-tone-4f6f89); border-radius: 4px; background: var(--color-tone-243747); color: var(--color-accent-text); cursor: pointer; } }
.variable-row { display: grid; grid-template-columns: minmax(65px, .8fr) minmax(0, 1.3fr) 20px; align-items: start; gap: 6px; padding: 6px; border-bottom: 1px solid var(--color-bg-raised); background: var(--color-tone-222); font-size: 10px; code { overflow: hidden; color: var(--color-accent-text); text-overflow: ellipsis; } span { overflow-wrap: anywhere; color: var(--color-code); font-family: Consolas, monospace; &.error { color: var(--color-danger); } } button { border: 0; background: transparent; color: var(--color-text-faint); cursor: pointer; &:hover { color: var(--color-danger); } } &--local { grid-template-columns: minmax(65px, .8fr) minmax(0, 1.3fr); } }
.variable-row__toggle { width: 16px; padding: 0 !important; color: var(--color-accent-text) !important; font-size: 14px; text-align: left; }.variable-tree__children { border-left: 1px solid var(--color-border-input); margin-left: 12px; > div { display: grid; grid-template-columns: minmax(55px,.55fr) 1fr; gap: 8px; padding: 5px 8px; border-bottom: 1px solid var(--color-tone-2d2d2d); background: var(--color-tone-1b1b1b); font: 10px Consolas,monospace; code { color: var(--color-tone-d16dce); } span { overflow-wrap: anywhere; color: var(--color-tone-b5cea8); } } }
.debug-panel__empty { padding: 10px 4px; color: var(--color-text-disabled); font-size: 10px; text-align: center; }
.debug-output pre { max-height: 140px; overflow: auto; margin: 0; padding: 7px; background: var(--color-bg-deep); color: var(--color-text-strong); font: 10px/1.45 Consolas, monospace; white-space: pre-wrap; &.error { color: var(--color-danger); } }
</style>
