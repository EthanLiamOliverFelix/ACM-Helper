...
.statistics-view { height: 100%; overflow-y: auto; background: var(--color-bg-deep); color: var(--color-text-primary); > header { position: sticky; top: 0; z-index: 2; display: flex; align-items: center; justify-content: space-between; padding: 18px 26px; border-bottom: 1px solid var(--color-tone-363636); background: var(--color-bg-panel-translucent); backdrop-filter: blur(8px); h2 { margin: 0; font-size: 20px; } p { margin: 5px 0 0; color: var(--color-text-muted); font-size: 11px; } button { border: 0; background: transparent; color: var(--color-text-soft); font-size: 27px; cursor: pointer; } } > main { width: min(1120px, calc(100% - 44px)); margin: 0 auto; padding: 24px 0 42px; } }
.summary-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; article { padding: 16px; border: 1px solid var(--color-tone-3b4750); border-radius: 8px; background: linear-gradient(145deg, var(--color-tone-252b30), var(--color-tone-222)); span, small { display: block; color: var(--color-tone-8fa0ab); font-size: 10px; } strong { display: block; margin: 7px 0 5px; color: var(--color-tone-75bfff); font-size: 25px; } small { color: var(--color-tone-707b82); } } }
.statistics-card { padding: 17px; border: 1px solid var(--color-tone-3a3a3a); border-radius: 9px; background: var(--color-tone-242424); }.activity-card { margin-top: 12px; }.card-heading { display: flex; justify-content: space-between; h3 { margin: 0; font-size: 14px; } p { margin: 4px 0 0; color: var(--color-tone-7f7f7f); font-size: 10px; } }
.activity-chart { display: grid; grid-template-columns: repeat(14, 1fr); gap: 7px; height: 155px; margin-top: 16px; }.activity-day { display: grid; grid-template-rows: 16px 1fr 17px; min-width: 0; text-align: center; > span { color: var(--color-tone-69d498); font-size: 9px; } > div { position: relative; display: flex; align-items: flex-end; justify-content: center; min-height: 75px; border-bottom: 1px solid var(--color-tone-414141); i { display: block; width: min(24px, 72%); min-height: 2px; border-radius: 3px 3px 0 0; background: linear-gradient(var(--color-accent), var(--color-tone-27658f)); &.empty { background: var(--color-tone-3a3a3a); } } } small { overflow: hidden; padding-top: 5px; color: var(--color-text-faint); font-size: 8px; white-space: nowrap; } }
.detail-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-top: 12px; }.platform-list { display: grid; gap: 7px; margin-top: 14px; > div { display: grid; grid-template-columns: 1fr auto auto; gap: 15px; align-items: center; padding: 9px 10px; border-radius: 5px; background: var(--color-tone-1d1d1d); strong { font-size: 11px; } span { color: var(--color-tone-69d498); font-size: 11px; } small { min-width: 70px; color: var(--color-text-faint); font-size: 9px; text-align: right; } } }
.bar-list { display: grid; gap: 8px; margin-top: 14px; > div { display: grid; grid-template-columns: 58px 1fr 26px; gap: 8px; align-items: center; span, b { font-size: 9px; } b { color: var(--color-text-soft); text-align: right; } > div { height: 7px; overflow: hidden; border-radius: 4px; background: var(--color-bg-subtle); i { display: block; height: 100%; border-radius: inherit; background: var(--color-tone-c15d5d); } } } }.language-list > div > div i { background: var(--color-tone-9b7bc1); }
.review-card > div:last-child { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; margin-top: 14px; article { padding: 13px 8px; border-radius: 6px; background: var(--color-tone-1d1d1d); text-align: center; strong, span { display: block; } strong { color: var(--color-tone-e0bd63); font-size: 20px; } span { margin-top: 4px; color: var(--color-text-muted); font-size: 9px; } } }.empty-state { padding: 20px 0; color: var(--color-text-disabled); font-size: 10px; text-align: center; }.statistics-note { margin: 14px 2px 0; color: var(--color-tone-707070); font-size: 9px; line-height: 1.5; }
