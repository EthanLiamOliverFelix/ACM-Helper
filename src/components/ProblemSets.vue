...
import type { LuoguTrainingCategory, LuoguTrainingPage } from '../types'

const sets = useProblemSetStore()
const problems = useProblemStore()
const learning = useLearningStore()
const practice = usePracticeStore()
const view = ref<'list' | 'detail' | 'smart' | 'plaza'>('list')
...
const smartSets = computed(() => [
  { id: 'wrongbook' as const, name: '自动错题本', description: '只收录真实失败且尚需巩固的题目', count: practice.wrongProblems.length },
  { id: 'today' as const, name: '今日复习', description: `按风险选取，今日最多 ${practice.dailyLimit} 题`, count: practice.todayProblems.length },
])
const filteredSmartSets = computed(() => {
  const query = setSearch.value.trim().toLowerCase()
  return query ? smartSets.value.filter((set) => set.name.toLowerCase().includes(query)) : smartSets.value
})
const activeSmart = computed(() => smartSets.value.find((set) => set.id === activeSmartId.value)!)
...
const filteredProblems = computed(() => {
  const query = problemSearch.value.trim().toLowerCase()
  const list = sets.activeSet?.problems ?? []
  return query ? list.filter((problem) => problem.id.toLowerCase().includes(query) || problem.title.toLowerCase().includes(query) || problem.tags.some((tag) => tag.toLowerCase().includes(query))) : list
})
const solvedCount = computed(() => sets.activeSet?.problems.filter((problem) => learning.profile.solvedProblems.includes(`${problem.platform}:${problem.id}`)).length ?? 0)
  ...
  <div class="problem-sets">
    <template v-if="view === 'list'">
      <header class="panel-header">
        <div><strong>我的题单</strong><span>{{ sets.sets.length }} 个普通题单 · 2 个智能题单</span></div>
      ...
      <div v-if="!bulkMode && filteredSmartSets.length" class="smart-grid">
        <button v-for="set in filteredSmartSets" :key="set.id" class="smart-card" @click="openSmart(set.id)">
          <span>{{ set.id === 'wrongbook' ? '⌁' : '◷' }}</span>
          <div><strong>{{ set.name }}</strong><small>{{ set.description }}</small></div>
          <b>{{ set.count }}</b>
        </button>
      </div>
      <button class="plaza-entry" @click="openPlaza"><span>洛谷题单广场</span><small>官方、教材与精选用户题单</small><b>›</b></button>
      ...
      <div class="set-grid">
        <button v-for="set in filteredSets" :key="set.id" class="set-card" :class="{ selected: selectedSetIds.has(set.id), dragging: dragId === set.id, 'drag-over': dragOverId === set.id }" :data-set-id="set.id" @click="openSet(set.id)" @pointerdown="beginHold(set.id, $event)" @contextmenu.prevent>
          ...
          <div><strong>{{ set.name }}</strong><span>{{ set.problems.length }} 道 · {{ set.problems.filter((problem) => learning.profile.solvedProblems.includes(`${problem.platform}:${problem.id}`)).length }} 已完成</span></div>
    ...
    <template v-else-if="view === 'smart'">
      <header class="panel-header detail-header"><button class="back" @click="view = 'list'">‹ 返回</button><div><strong>{{ activeSmart.name }}</strong><span>{{ activeSmart.count }} 道</span></div><div class="detail-header__actions"><button class="tag-toggle" :class="{ active: problems.showProblemTags }" @click="problems.toggleProblemTags">{{ problems.showProblemTags ? '隐藏算法标签' : '显示算法标签' }}</button></div></header>
      ...
      <ul class="problem-items practice-items">
        <li v-for="problem in filteredSmartProblems" :key="problem.practice.key" @click="practice.openProblem(problem)">
          ...
          <div>
            <small>{{ problem.platform === 'codeforces' ? 'CF' : problem.platform === 'atcoder' ? 'AtCoder' : problem.platform === 'qoj' ? 'QOJ' : '洛谷' }} · {{ problem.id }}</small>
    ...
    <template v-else-if="view === 'detail' && sets.activeSet">
      ...
      <div class="detail-progress-label"><span>{{ solvedCount }}/{{ sets.activeSet.problems.length }} 已完成</span><em v-if="metadataLoading">正在补全题目信息…</em></div>
      <div class="progress"><i :style="{ width: `${sets.activeSet.problems.length ? solvedCount / sets.activeSet.problems.length * 100 : 0}%` }" /></div>
...
.detail-header { justify-content: flex-start; > div:not(.detail-header__actions) { flex: 1; } .back { flex: 0 0 auto; color: #9cdcfe; } &__actions { display: flex; flex: 0 0 auto; flex-direction: row !important; gap: 4px; } .tag-toggle.active { border-color: #4d718f; background: #203545; color: #9cdcfe; } }.detail-progress-label { display: flex; align-items: center; justify-content: space-between; padding: 4px 10px 3px; color: #777; font-size: 9px; em { color: #9cdcfe; font-style: normal; } }.new-set, .training-link { display: flex; gap: 5px; padding: 8px 10px; input { min-width: 0; flex: 1; padding: 6px 7px; border: 1px solid #444; border-radius: 4px; background: #252526; color: #ddd; font-size: 10px; outline: none; &:focus { border-color: #569cd6; } } button { padding: 0 9px; border: 0; border-radius: 4px; background: #0e639c; color: white; font-size: 9px; cursor: pointer; &:disabled { opacity: .4; } } }
.smart-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 5px; padding: 0 10px 7px; }.smart-card { display: grid; grid-template-columns: auto 1fr auto; align-items: center; gap: 6px; min-width: 0; padding: 8px; border: 1px solid #456b5b; border-radius: 6px; background: #20332b; color: #cdebdc; text-align: left; cursor: pointer; > span { color: #4ec982; font-size: 16px; } div { display: flex; min-width: 0; flex-direction: column; gap: 2px; } strong { overflow: hidden; font-size: 10px; text-overflow: ellipsis; white-space: nowrap; } small { overflow: hidden; color: #7fa895; font-size: 7px; line-height: 1.3; } b { color: #4ec982; font-size: 15px; } &:hover { border-color: #4ec982; background: #244034; } }
.plaza-entry { display: grid; grid-template-columns: 1fr auto; grid-template-rows: auto auto; margin: 0 10px 7px; padding: 9px 10px; border: 1px solid #4d718f; border-radius: 6px; background: #203545; color: #d7efff; text-align: left; cursor: pointer; span { font-size: 12px; } small { color: #82a9c2; font-size: 9px; } b { grid-column: 2; grid-row: 1 / 3; align-self: center; font-size: 20px; } }.bulk-bar { display: flex; align-items: center; gap: 7px; padding: 6px 10px; border-block: 1px solid #3a3a3a; background: #252526; font-size: 9px; span { flex: 1; color: #888; } button { padding: 3px 6px; border: 1px solid #444; border-radius: 3px; background: #333; color: #aaa; cursor: pointer; &.danger { border-color: #6a3939; color: #f48771; } &:disabled { opacity: .4; } } }
