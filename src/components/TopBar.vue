  ...
  <div v-if="settingsOpen" class="settings-modal" @click.self="settingsOpen = false">
    <section class="settings-card">
      ...
      <div v-if="settingsPage === 'main'" class="settings-grid">
        ...
        <section>
          <h3>外观</h3>
          <label>色彩模式<select v-model="settings.theme"><option value="system">跟随系统</option><option value="dark">深色</option><option value="light">浅色</option></select></label>
          <p class="privacy">切换后立即生效，并同步调整代码编辑器配色。</p>
          <h3 class="account-title">AI 模型</h3>
...
.language-tabs, .assist-levels { display: flex; gap: 5px; margin-bottom: 8px; button { padding: 6px 9px; border: 1px solid var(--color-border-control); border-radius: 4px; background: var(--color-bg-app); color: var(--color-text-soft); cursor: pointer; &.active { border-color: var(--color-accent); background: var(--color-selection); color: var(--color-text-on-accent); } } }
.ai-entry-list { display: grid; gap: 8px; margin-bottom: 20px; > button { display: grid; grid-template-columns: 34px 1fr auto; align-items: center; gap: 10px; padding: 12px; border: 1px solid var(--color-tone-465968); border-radius: 7px; background: var(--color-tone-1d2931); color: var(--color-text-strong); text-align: left; cursor: pointer; &:hover { border-color: var(--color-accent); background: var(--color-accent-surface); } > span { display: grid; place-items: center; width: 32px; height: 32px; border-radius: 7px; background: var(--color-selection); color: var(--color-tone-d8efff); font-size: 13px; font-weight: 700; } > div { display: flex; min-width: 0; flex-direction: column; gap: 4px; } strong { font-size: 12px; } small { overflow: hidden; color: var(--color-tone-8295a3); font-size: 9px; text-overflow: ellipsis; white-space: nowrap; } b { color: var(--color-accent-text); font-size: 10px; } } }
.ai-model-settings { max-width: 720px; min-height: 390px; margin: 0 auto; padding: 28px 24px; label { display: block; margin-bottom: 13px; color: var(--color-tone-999); font-size: 10px; } input, select { box-sizing: border-box; width: 100%; margin-top: 5px; padding: 9px; border: 1px solid var(--color-border-control); border-radius: 4px; outline: none; background: var(--color-bg-deep); color: var(--color-text-strong); &:focus { border-color: var(--color-accent); } } h3 { margin: 22px 0 10px; font-size: 13px; } }
.model-picker { display: flex; gap: 7px; margin-top: 5px; input { min-width: 0; margin-top: 0 !important; } button { flex: 0 0 auto; padding: 0 12px; border: 1px solid var(--color-tone-4b6274); border-radius: 4px; background: var(--color-accent-surface); color: var(--color-accent-text); cursor: pointer; &:disabled { cursor: default; opacity: .45; } } }
.model-list-status { margin: -3px 0 13px; padding: 8px 10px; border-radius: 4px; font-size: 10px; line-height: 1.45; &.success { background: var(--color-tone-173126); color: var(--color-tone-73d49f); } &.warning { background: var(--color-tone-3b321d); color: var(--color-tone-e8c56b); } &.error { background: var(--color-tone-3f2020); color: var(--color-danger); } }
.privacy { padding: 8px; background: var(--color-bg-app); color: var(--color-text-muted); font-size: 10px; line-height: 1.5; }.primary, .secondary { padding: 7px 13px; border: 0; border-radius: 4px; color: var(--color-text-on-accent); cursor: pointer; }.primary { background: var(--color-accent-strong); }.secondary { background: var(--color-border-control); font-size: 10px; }
.runner-settings-title { margin-top: 20px !important; }
.checkbox-label { display: flex !important; align-items: center; gap: 7px; color: var(--color-tone-ccc) !important; input { width: auto; margin: 0; } }.account-title { margin-top: 20px !important; }.account-row { display: flex; align-items: center; justify-content: space-between; gap: 10px; padding: 8px 9px; border-bottom: 1px solid var(--color-tone-3b3b3b); background: var(--color-bg-app); font-size: 11px; > div { display: flex; min-width: 0; flex-direction: column; gap: 3px; } strong { overflow: hidden; color: var(--color-success); text-overflow: ellipsis; white-space: nowrap; } strong.offline { color: var(--color-text-muted); font-weight: 400; } button { flex: 0 0 auto; padding: 5px 8px; border: 1px solid var(--color-tone-4b6274); border-radius: 4px; background: var(--color-accent-surface); color: var(--color-accent-text); font-size: 9px; cursor: pointer; &:disabled { opacity: .4; } } }
.path-picker { display: flex; gap: 6px; margin-top: 4px; input { min-width: 0; margin-top: 0 !important; } button { flex: 0 0 auto; padding: 0 10px; border: 1px solid var(--color-tone-4b6274); border-radius: 4px; background: var(--color-accent-surface); color: var(--color-accent-text); cursor: pointer; &:disabled { opacity: .45; } } }
.toolchain-websites { display: flex; flex-wrap: wrap; align-items: center; gap: 6px; margin-top: 10px; padding: 8px; border: 1px solid var(--color-tone-3b3b3b); border-radius: 5px; background: var(--color-bg-app); span { width: 100%; color: var(--color-text-muted); font-size: 10px; } button { padding: 5px 8px; border: 1px solid var(--color-tone-4b6274); border-radius: 4px; background: var(--color-accent-surface); color: var(--color-accent-text); font-size: 9px; cursor: pointer; } }
.data-center-settings { margin: 0 20px 20px; padding: 15px; border: 1px solid var(--color-tone-3f5260); border-radius: 7px; background: var(--color-tone-1d252b); label { display: block; color: var(--color-text-soft); font-size: 10px; } input { box-sizing: border-box; width: 100%; padding: 8px; border: 1px solid var(--color-border-control); border-radius: 4px; outline: none; background: var(--color-bg-deep); color: var(--color-text-strong); } &__heading { display: flex; justify-content: space-between; gap: 20px; margin-bottom: 12px; h3 { margin: 0 0 4px; font-size: 14px; } p { margin: 0; color: var(--color-tone-8d9aa3); font-size: 10px; } span { flex: 0 0 auto; color: var(--color-tone-7fc8f3); font-size: 10px; } } &__actions { display: flex; justify-content: flex-end; gap: 8px; margin-top: 10px; }.migrate { padding: 7px 12px; border: 0; border-radius: 4px; background: var(--color-accent-strong); color: var(--color-text-on-accent); cursor: pointer; &:disabled { opacity: .45; cursor: default; } } &__success { margin: 8px 0 0; color: var(--color-tone-65c68b); font-size: 10px; } &__error { margin: 8px 0 0; color: var(--color-danger); font-size: 10px; } }
.data-center-tools { display: flex; gap: 7px; margin-top: 10px; button { padding: 6px 10px; border: 1px solid var(--color-tone-4b6274); border-radius: 4px; background: var(--color-accent-surface); color: var(--color-accent-text); cursor: pointer; &:disabled { opacity: .45; } } }
.backup-list { display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 6px; margin-top: 10px; > span { grid-column: 1 / -1; color: var(--color-tone-8d9aa3); font-size: 10px; } button { display: flex; min-width: 0; flex-direction: column; gap: 2px; padding: 7px 9px; border: 1px solid var(--color-tone-3d4b54); border-radius: 4px; background: var(--color-tone-182027); color: var(--color-tone-ccc); text-align: left; cursor: pointer; b { overflow: hidden; font-size: 10px; text-overflow: ellipsis; white-space: nowrap; } small { color: var(--color-tone-7f929e); font-size: 9px; } } }
.diagnostic-settings { display: grid; grid-template-columns: 1fr auto; gap: 10px 20px; margin: 0 20px 20px; padding: 15px; border: 1px solid var(--color-border-control); border-radius: 7px; background: var(--color-bg-panel-alt); h3 { margin: 0 0 4px; font-size: 14px; } p { margin: 0; color: var(--color-tone-8d8d8d); font-size: 10px; } &__actions { display: flex; gap: 6px; button { padding: 6px 9px; border: 1px solid var(--color-tone-4b6274); border-radius: 4px; background: var(--color-accent-surface); color: var(--color-accent-text); cursor: pointer; &:disabled { opacity: .4; } } } }
.diagnostic-list { grid-column: 1 / -1; display: grid; gap: 4px; div { display: grid; grid-template-columns: 140px 160px 1fr; gap: 8px; padding: 6px 8px; border-left: 3px solid var(--color-success); background: var(--color-bg-deep); color: var(--color-text-soft); font-size: 9px; &.error { border-left-color: var(--color-danger); } b { color: var(--color-tone-ccc); } em { overflow: hidden; color: var(--color-tone-8d9aa3); font-style: normal; text-overflow: ellipsis; white-space: nowrap; } } }
